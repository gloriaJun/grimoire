...
5. Always confirm before irreversible external actions: push, PR/Issue/Discussion creation, messages, deleting/moving 3+ files, `rm -rf`. Approval expires when diff, commits, branch, target, or plan changes. Before `git push`, show branch status, unpushed commits, and diff/stat.
