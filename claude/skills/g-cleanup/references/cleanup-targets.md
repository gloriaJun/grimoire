...
| C7 | History | `history.jsonl` | Command history (truncate only) |
...
| ID | Category | Path | Description |
|----|----------|------|-------------|
| M1 | Memory audit | `projects/*/memory/` | Auto-memory files - audit for orphaned project paths, broken MEMORY.md indexes, and duplicate entries across renamed project dirs. Never deleted blindly; every issue needs user confirmation. |

## Temp files (`/tmp/`)

| ID | Category | Pattern | Description |
|----|----------|---------|-------------|
| T1 | Claude temp | `/tmp/claude-*` | Claude Code temp dirs, including session scratchpads. Retention (mtime) applies; never delete entries modified within the retention window. |
