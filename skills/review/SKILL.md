...
name: review
description: >
  On-demand code review skill. Triggered by /dev review or natural language:
  "이 부분 검토해줘", "이 코드 리뷰해줘", "이 코드 봐줘", "코드 개선 제안해줘",
  "이 부분 수정 제안해줘", "review this", "check this code".
  Routes reviewer based on authorship (Claude impl → Codex reviews, Codex impl → Claude reviews).
  Does not require an active devlog. Also invoked by /dev as a thin wrapper.
---
