...
5. For `M1` (Memory audit): re-run `scripts/audit-memory.sh` and for each issue:
   - **Preserve guard (run first)**: if any file in a deletion-target `memory/` directory has `preserve: true` frontmatter, do NOT auto-delete — confirm with the user individually.
